...
void ParticleSystem::setup(float _width, float _height, int numParticles)
{
    ...
    parameters.add( life.set(           "life",     200, 1, 1000));
...
void ParticleSystem::update()
{
    ...
    for (int i = 0; i < particleData.countAlive; i++)
    {
        ...
        if (p.elapsedLife > p.maxLife
            // NEW:
            // we also kill particles when they are out of bounds
            || p.pos.x < 0.0
            || p.pos.x > width
            || p.pos.y < 0
            || p.pos.y > height)
...
{
    emitters.push_back(emitter); //< store pointer
}


void ParticleSystem::addForce(ForceAttractor* force)
{
    forces.push_back(force); // store pointer
}
